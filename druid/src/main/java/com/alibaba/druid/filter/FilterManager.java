...
public class FilterManager {
    ...
    private static void loadFilterConfig(Properties filterProperties, ClassLoader classLoader)
                                                                                              throws IOException {
        if (classLoader == null) {
            return;
        }

        for (Enumeration<URL> e = classLoader.getResources("META-INF/druid-filter.properties"); e
            .hasMoreElements();) {
    ...
    public static void loadFilter(List<Filter> filters, String filterName) throws SQLException {
        ...
        if (filterClassNames != null) {
            for (String filterClassName : filterClassNames.split(",")) {
                ...
                try {
                    ...
                    throw new SQLException("load managed jdbc driver event listener error. "
                                           + filterName, e);
        ...
        try {
            ...
            throw new SQLException("load managed jdbc driver event listener error. " + filterName,
                e);
        }
    }
