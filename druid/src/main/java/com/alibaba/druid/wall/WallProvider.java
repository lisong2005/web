...
public abstract class WallProvider {
    ...
    private WallCheckResult checkInternal(String sql) {
        ...
        boolean mulltiTenant = config.getTenantTablePattern() != null
                               && config.getTenantTablePattern().length() > 0;
        if (!mulltiTenant) {
        ...
        try {
            ...
            if (lastToken != Token.EOF && config.isStrictSyntaxCheck()) {
                violations.add(new IllegalSQLObjectViolation(ErrorCode.SYNTAX_ERROR,
                    "not terminal sql, token " + lastToken, sql));
            }
            endOfComment = parser.getLexer().isEndOfComment();
        } catch (NotAllowCommentException e) {
            violations.add(new IllegalSQLObjectViolation(ErrorCode.COMMENT_STATEMENT_NOT_ALLOW,
                "comment not allow", sql));
        ...
        if (statementList.size() > 1 && !config.isMultiStatementAllow()) {
            violations.add(new IllegalSQLObjectViolation(ErrorCode.MULTI_STATEMENT,
                "multi-statement not allow", sql));
        }
        ...
        if (violations.size() > 0) {
            ...
            if (sql.length() < MAX_SQL_LENGTH) {
                sqlStat = addBlackSql(sql, context.getTableStats(), context.getFunctionStats(),
        ...
        if (sqlStat != null) {
            ...
            result = new WallCheckResult(null, violations, tableStats, functionStats,
                statementList, syntaxError);
        }
