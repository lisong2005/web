...
public class MySqlWallVisitor extends MySqlASTVisitorAdapter implements WallVisitor,
    ...
    public boolean visit(SQLSelectStatement x) {
        if (!config.isSelelctAllow()) {
            this.getViolations().add(
                new IllegalSQLObjectViolation(ErrorCode.SELECT_NOT_ALLOW, "select not allow", this
                    .toSQL(x)));
            return false;
        }
        ...
        if (x.getRowCount() instanceof SQLNumericLiteralExpr) {
            ...
            if (rowCount == 0) {
                ...
                if (!provider.getConfig().isLimitZeroAllow()) {
                    this.getViolations().add(
                        new IllegalSQLObjectViolation(ErrorCode.LIMIT_ZERO, "limit row 0", this
                            .toSQL(x)));
                }
            }
        }
            ...
            if (varName.equalsIgnoreCase("@@session") || varName.equalsIgnoreCase("@@global")) {
                ...
                if (!checkVar(x.getParent(), x.getName())) {
                    ...
                    if (!isTop) {
                        boolean allow = true;
                        if (isDeny(varName)
                            && (WallVisitorUtils.isWhereOrHaving(x) || WallVisitorUtils
                                .checkSqlExpr(varExpr))) {
                            allow = false;
                        }
    ...
    public boolean visit(SQLVariantRefExpr x) {
        ...
        if (varName.startsWith("@@") && !checkVar(x.getParent(), x.getName())) {
            ...
            if (!isTop) {
                boolean allow = true;
                if (isDeny(varName)
                    && (WallVisitorUtils.isWhereOrHaving(x) || WallVisitorUtils.checkSqlExpr(x))) {
                    allow = false;
                }
    ...
    public boolean visit(MySqlOutFileExpr x) {
        if (!config.isSelectIntoOutfileAllow() && !WallVisitorUtils.isTopSelectOutFile(x)) {
            violations.add(new IllegalSQLObjectViolation(ErrorCode.INTO_OUTFILE,
                "into out file not allow", toSQL(x)));
        }
