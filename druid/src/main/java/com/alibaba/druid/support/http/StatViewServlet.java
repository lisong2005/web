    ...
    public void init() throws ServletException {
        ...
        try {
            ...
            String msg = "initParameter config error, resetEnable : "
                         + getInitParameter(PARAM_NAME_RESET_ENABLE);
            LOG.error(msg, e);
        }
            ...
            if (conn == null) {// 连接在初始化时创建失败
                ...
                if (conn != null) {// 连接成功
                    try {
                        ...
                        resp = DruidStatService.returnJSONResult(
                            DruidStatService.RESULT_CODE_ERROR, "get data error:" + e.getMessage());
                    }
                }
            } else {// 连接成功
                try {
