...
public class LogFactory {
    ...
    static {
        ...
        System.out.println(String.format("【logtype=%s】", logType));
        if (logType != null) {
            if (logType.equalsIgnoreCase("slf4j")) {
                ...
                tryImplementation("org.apache.logging.log4j.Logger",
        ...
        if (LOG != null && LOG.isDebugEnabled()) {
            LOG.debug(String.format("[%s]", logConstructor));
        }
    ...
    private static void tryImplementation(String testClassName, String implClassName) {
        if (LOG != null && LOG.isDebugEnabled()) {
            LOG.debug(String.format("[%s, %s]", testClassName, implClassName));
        }
        if (logConstructor != null) {
            return;
        }
        System.out.println(String.format("[%s, %s]", testClassName, implClassName));
        ...
        try {
            ...
            try {
                if (null != logConstructor) {
                    LOG = (Log) logConstructor.newInstance(LogFactory.class.getName());
                }
            } catch (Throwable t) {
                logConstructor = null;
            }
    ...
    public static Log getLog(String loggerName) {
        if (LOG != null && LOG.isDebugEnabled()) {
            LOG.debug(String.format("%s", loggerName));
        }
        try {
            return (Log) logConstructor.newInstance(loggerName);
        } catch (Throwable t) {
            throw new RuntimeException("Error creating logger for logger '" + loggerName
                                       + "'.  Cause: " + t, t);
        }
    }
    ...
    public static synchronized void selectJavaLogging() {
        try {
            ...
            Class implClass = Resources
                .classForName("com.alibaba.druid.support.logging.Jdk14LoggingImpl");
            logConstructor = implClass.getConstructor(new Class[] { String.class });
        } catch (Throwable t) {
            //ignore
        }
