...
public class MySqlExprParser extends SQLExprParser {
    ...
    public static String[] AGGREGATE_FUNCTIONS = { "AVG", "COUNT", "GROUP_CONCAT", "MAX", "MIN",
            "STDDEV", "SUM"                   };
        ...
        switch (tok) {
            ...
            case VARIANT:
                ...
                if (varRefExpr.getName().equalsIgnoreCase("@@global")) {
                    ...
                    lexer.nextToken();
                } else if (varRefExpr.getName().equals("@@")
    ...
    public final SQLExpr primaryRest(SQLExpr expr) {
                ...
                if (lexer.token() == Token.IN) {
                    ...
                    if (identifierEquals("NATURAL")) {
                        ...
                        if (lexer.token() == Token.WITH) {
                            ...
                            matchAgainstExpr
                                .setSearchModifier(SearchModifier.IN_NATURAL_LANGUAGE_MODE_WITH_QUERY_EXPANSION);
                        } else {
                            matchAgainstExpr
                                .setSearchModifier(SearchModifier.IN_NATURAL_LANGUAGE_MODE);
                        }
        ...
        if (lexer.token() == Token.ERROR) {
            throw new ParserException("syntax error, token: " + lexer.token() + " "
                                      + lexer.stringVal() + ", pos : " + lexer.pos());
        }
    ...
    public MysqlForeignKey parseForeignKey() {
        ...
        if (lexer.token() == Token.ON) {
            ...
            if (lexer.token() == Token.DELETE) {
                ...
                throw new ParserException("syntax error, expect DELETE or UPDATE, actual "
            ...
            if (lexer.token() == Token.RESTRICT) {
                ...
                if (identifierEquals("ACTION")) {
                    ...
                    throw new ParserException("syntax error, expect ACTION, actual "
                                              + lexer.token() + " " + lexer.stringVal());
                }
            }
            lexer.nextToken();
