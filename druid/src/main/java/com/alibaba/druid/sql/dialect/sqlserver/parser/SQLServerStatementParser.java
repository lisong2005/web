    ...
    protected void parseInsert0(SQLInsertInto insert, boolean acceptSubQuery) {
        ...
        if (lexer.token() == Token.VALUES) {
            ...
            for (;;) {
                ...
                if (!parseCompleteValues
                    && insertStatement.getValuesList().size() >= parseValuesSize) {
                    lexer.skipToEOF();
                    break;
                }
    ...
    public SQLStatement parseSet() {
        ...
        if (identifierEquals("STATISTICS")) {
            ...
            if (identifierEquals("IO") || identifierEquals("XML") || identifierEquals("PROFILE")
                ...
                stmt.getItem().setTarget(
                    new SQLIdentifierExpr("STATISTICS " + lexer.stringVal().toUpperCase()));
