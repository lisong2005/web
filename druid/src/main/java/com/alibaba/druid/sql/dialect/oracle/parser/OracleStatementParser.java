    ...
    public void parseStatementList(List<SQLStatement> statementList, int max) {
        for (;;) {
            ...
            if (lexer.token() == (Token.SELECT)) {
                SQLSelectStatement stmt = new SQLSelectStatement(new OracleSelectParser(
                    this.exprParser).select(), JdbcConstants.ORACLE);
            ...
            if (lexer.token() == Token.WITH) {
                statementList.add(new SQLSelectStatement(new OracleSelectParser(this.exprParser)
                    .select()));
                continue;
            }
        ...
        for (;;) {
            if (identifierEquals("ADD")) {
                ...
                if (lexer.token() == Token.LPAREN) {
                    ...
                    OracleConstraint constraint = ((OracleExprParser) this.exprParser)
                        .parseConstaint();
