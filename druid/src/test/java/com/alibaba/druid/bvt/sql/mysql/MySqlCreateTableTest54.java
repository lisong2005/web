        ...
        Assert
            .assertEquals(
                "CREATE TABLE `t_activity_node` ("
                        ...
                        + "\n\t`end_time` datetime NULL COMMENT '该节点活动的结束时间', "
                        + "\n\t`activity_start_time` datetime NULL COMMENT '营销活动的开始时间', "
                        + "\n\t`activity_end_time` datetime NULL COMMENT '营销活动的结束时间', "
                        + "\n\t`report_start_time` datetime NULL COMMENT '该节点活动效果报告的开始时间', "
                        + "\n\t`report_end_time` datetime NULL COMMENT '该节点活动效果报告的结束时间', "
                        + "\n\t`cron_rule` varchar(100) NULL COMMENT '周期性营销的时间表达式', "
                        + "\n\t`split_rule` varchar(1000) NULL COMMENT '节点拆分规则', "
                        + "\n\t`search_json` varchar(5000) NULL COMMENT '索引查找的条件字符串', "
                        + "\n\t`filter_json` varchar(1000) NULL COMMENT '各种过滤条件的设置', "
                        + "\n\t`buyer_count` int(11) NULL COMMENT '营销的会员数量', "
                        + "\n\t`gmt_modified` datetime NOT NULL COMMENT '活动最后修改时间', "
                        + "\n\t`gmt_create` datetime NOT NULL COMMENT '活动创建时间', "
                        + "\n\tPRIMARY KEY (`id`), "
                        + "\n\tKEY `canvas_id` (`canvas_id`), "
