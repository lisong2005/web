...
public class TestIdle extends TestCase {
    ...
    protected void setUp() throws Exception {
        ...
        if (dataSourceList.size() > 0) {
            DruidDataSource first = DruidDataSourceStatManager.getInstance()
                .getDruidDataSourceInstances().iterator().next();
    ...
    public void test_idle() throws Exception {
...

        {
            ...
            dataSource.shrink();
            Assert.assertEquals("activeCount not zero", 0, dataSource.getActiveCount());
            Assert.assertEquals("minIdle not equal physical", dataSource.getMinIdle(), driver
                .getConnections().size());
        }
