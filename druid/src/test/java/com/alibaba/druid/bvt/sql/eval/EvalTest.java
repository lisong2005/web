...
public class EvalTest extends TestCase {
    ...
    public void testEval_1() throws Exception {
        Assert.assertEquals("AB",
            SQLEvalVisitorUtils.evalExpr(JdbcConstants.MYSQL, "? + ?", "A", "B"));
        Assert.assertEquals(234,
            SQLEvalVisitorUtils.evalExpr(JdbcConstants.MYSQL, "? + ?", 123, 111));
    }
    ...
    public void testEval_3() throws Exception {
        Assert.assertEquals(new BigDecimal("110"), SQLEvalVisitorUtils.evalExpr(
            JdbcConstants.MYSQL, "? * ?", new BigDecimal("10"), new BigDecimal("11")));
    }
