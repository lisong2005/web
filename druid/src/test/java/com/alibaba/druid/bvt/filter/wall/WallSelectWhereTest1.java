...
public class WallSelectWhereTest1 extends TestCase {
    ...
    public void testORACLE_true() throws Exception {
        Assert.assertTrue(WallUtils
            .isValidateOracle("SELECT F1, F2 from t WHERE 1 = 1 AND FID = ?"));
    }

    public void testMySql_false() throws Exception {
        Assert.assertFalse(WallUtils
            .isValidateMySql("SELECT F1, F2 from t WHERE 1 = 1 AND FID = ? OR 1 = 1"));
    }

    public void testORACLE_false() throws Exception {
        Assert.assertFalse(WallUtils
            .isValidateOracle("SELECT F1, F2 from t WHERE 1 = 1 AND FID = ? OR 1 = 1"));
    }
}
