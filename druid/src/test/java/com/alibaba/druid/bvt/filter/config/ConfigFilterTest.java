...
public class ConfigFilterTest extends ConfigFileGenerator {
    ...
    public void testInitRemoteConfigFile() throws SQLException {
        ...
        try {
            ...
            Assert.assertEquals("The username is " + dataSource.getUsername(), "test1",
    ...
    public void testInitDecrypt() throws SQLException {
        ...
        try {
            dataSource.init();
            Assert.assertEquals("The password is " + dataSource.getPassword() + ", is not xiaoyu",
                "xiaoyu", dataSource.getPassword());
        } finally {
            JdbcUtils.close(dataSource);
        }
    }
    ...
    public void testInitRemoteConfigAndDecrypt() throws SQLException {
        DruidDataSource dataSource = new DruidDataSource();
        dataSource.setFilters("config");
        dataSource.setConnectionProperties("config.decrypt=true;config.file=" + "file://"
                                           + this.filePath);
        try {
            dataSource.init();

            Assert.assertEquals("The password is " + dataSource.getPassword(), "xiaoyu",
